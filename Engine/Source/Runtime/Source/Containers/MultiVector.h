...
namespace Lumina
{
    ...
    public:
        ...
        {
            Lumina::Get<TVector<std::decay_t<T>>>(Tuple).emplace(std::forward<TArgs>(Args)...);
        }

        template<typename T>
        void PushBack(T&& Val)
        {
            Lumina::Get<TVector<std::decay_t<T>>>(Tuple).push_back(std::forward<T>(Val));
        }
    ...
    private:
        ...
        {
            TVector<T>& Vector = Lumina::Get<TVector<T>>(Tuple);
