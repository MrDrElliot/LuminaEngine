...
namespace Lumina
{
                    ...
                    if (bVertexSnapArmed)
                    {
                        ...
                        if (bHaveAnchor)
                        {
                            ...
                            if (ProjectPointToScreen(AnchorWorld, SnapViewProj, ViewportSize, S))
                            {
                                ...
                                DL->AddRect(ImVec2(P.x - 7, P.y - 7), ImVec2(P.x + 7, P.y + 7), C, 0.0f, 2.0f, 0);
                            }
                        }
        ...
        if (!bModeOwnsInput && !bCameraOwnsInput && !bCameraPreviewMouseOver && !AreVisualizersCapturingInput() && ImGui::IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows) && !RmlUi::WorldUIWantsMouse(World))
        {
            ...
            if (!PickCtx.Broker->IsRequested() && !bOverImGuizmo)
            {
                ...
                if (SelectionBox.bActive && bLeftDragging)
                {
                    ...
                    DrawList->AddRect(ScreenStart, ScreenEnd, IM_COL32(100, 150, 255, 255), 0.0f, 2.0f, 0);
                }
    ...
    void FWorldEditorTool::DrawTagList(ECS::FEntity Entity)
    {
        ...
        DrawList->AddRect(CursorPos, ImVec2(CursorPos.x + HeaderSize.x, CursorPos.y + HeaderSize.y), IM_COL32(45, 45, 52, 255), 6.0f, 1.0f, 0);
        ...
        for (const FName& Tag : Tags)
        {
            ...
            DrawList->AddRect(ChipPos, ImVec2(ChipPos.x + ChipSize.x, ChipPos.y + ChipSize.y), ChipBorder, 12.0f, 1.0f, 0);
