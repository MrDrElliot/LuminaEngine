...
namespace Lumina
{
    ...
    CFactory* CFactory::FindReimportFactory(const CStruct* AssetClass)
    {
        ...
        {
            if (Factory != nullptr && Factory->CanReimport(AssetClass))
            {
                return Factory;
            }
        }
    ...
    FFixedString CFactory::BuildFileDialogFilter() const
    {
        ...
        // Win32 filters are a double-null-terminated run of null-separated pairs, so the string is built
        // with explicit embedded nulls rather than by concatenating C strings.
